...
// controller reconciling v1.Lease.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.Lease. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.Lease) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Lease.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.Lease. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Lease) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Lease if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Lease.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.Lease) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.Lease) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.Lease resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client kubernetes.Interface, lister coordinationv1.LeaseLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Leases(s.namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Lease, desiredFinalizers sets.Set[string]) (*v1.Lease, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.CoordinationV1().Leases(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Lease) (*v1.Lease, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Lease, reconcileEvent reconciler.Event) (*v1.Lease, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
