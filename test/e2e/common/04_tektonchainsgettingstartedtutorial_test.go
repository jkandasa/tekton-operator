...
func (s *TektonChainTutorialTestSuite) SetupSuite() {
	...
	s.logger.Debug("delete existing chain cr, if any")
	s.deleteChainCR()
...
func (s *TektonChainTutorialTestSuite) TearDownSuite() {
	s.logger.Debug("removing tekton chain cr")
	s.deleteChainCR()
}
...
func (s *TektonChainTutorialTestSuite) Test01() {
	...
	// Delete the TektonChain CR instance
	t.Run("delete-chain-cr", func(t *testing.T) {
		s.deleteChainCR()
	})
}
